...
#define dq_rem(p, q) \
  do \
    { \
        dq_entry_t *tmp_node = (p); \
        dq_entry_t *tmp_prev = tmp_node->blink; \
        dq_entry_t *tmp_next = tmp_node->flink; \
      if (!tmp_prev) \
        { \
          (q)->head = tmp_next; \
        } \
